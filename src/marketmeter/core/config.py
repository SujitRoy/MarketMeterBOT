"""
MarketMeter — core configuration.

All secrets from environment variables; constants centralized here.
This is the canonical location. A thin shim at /config.py re-exports these
symbols so existing call sites keep working until Phase 6 retires the shim.
"""
import os
...
# NOTE: closed-day classification lives in core.time.is_trading_day /
# core.time.NSE_HOLIDAYS, so a weekday exchange holiday is 'holiday' (skipped),
# never a retryable 'failed'.
...
TRADINGVIEW_SESSION_ID = os.getenv("MARKETMETER_TRADINGVIEW_SESSION_ID", "")
